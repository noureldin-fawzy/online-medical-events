...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.exhibitionDetail.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.exhibition-details.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('exhibition') ? 'is-invalid' : '' }}" name="exhibition_id" id="exhibition_id" required>
                    @foreach($exhibitions as $id => $exhibition)
                        <option value="{{ $id }}" {{ old('exhibition_id') == $id ? 'selected' : '' }}>{{ $exhibition }}</option>
                    @endforeach
                </select>
