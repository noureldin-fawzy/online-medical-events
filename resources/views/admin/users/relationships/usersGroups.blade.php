...
<div class="card">
    ...
    <div class="card-body">
        <div class="table-responsive">
            <table class=" table table-bordered table-striped table-hover datatable datatable-usersGroups">
                <thead>
                    <tr>
                        <th width="10">
                        ...
                        <th>
                            {{ trans('cruds.group.fields.active') }}
                        </th>
                        <th>
                            &nbsp;
                        </th>
                ...
                <tbody>
                    @foreach($groups as $key => $group)
                        <tr data-entry-id="{{ $group->id }}">
                            ...
                            <td>
                                <span style="display:none">{{ $group->active ?? '' }}</span>
                                <input type="checkbox" disabled="disabled" {{ $group->active ? 'checked' : '' }}>
                            </td>
                            <td>
                                @can('group_show')
...
@endcan
  ...
  let table = $('.datatable-usersGroups:not(.ajaxTable)').DataTable({ buttons: dtButtons })
