    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.contacts.update", [$contact->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('organizer') ? 'is-invalid' : '' }}" name="organizer_id" id="organizer_id" required>
                    @foreach($organizers as $id => $organizer)
                        <option value="{{ $id }}" {{ ($contact->organizer ? $contact->organizer->id : old('organizer_id')) == $id ? 'selected' : '' }}>{{ $organizer }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('branch') ? 'is-invalid' : '' }}" type="text" name="branch" id="branch" value="{{ old('branch', $contact->branch) }}" required>
            ...
            <div class="form-group">
                <label>{{ trans('cruds.contact.fields.active') }}</label>
                @foreach(App\Models\Contact::ACTIVE_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('active') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="active_{{ $key }}" name="active" value="{{ $key }}" {{ old('active', $contact->active) === (string) $key ? 'checked' : '' }}>
